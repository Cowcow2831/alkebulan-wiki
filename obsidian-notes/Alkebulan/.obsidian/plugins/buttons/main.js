...
var removeSection = async (app, section, buttonPosition) => {
  const { contentArray, file } = await createContentArray(app);
...
var createNote = async (app, type, folder, prompt, filePath, isTemplater) => {
  ...
  if (path) {
    ...
    try {
      ...
      const originalActiveView = app.workspace.getActiveViewOfType(import_obsidian7.MarkdownView);
      ...
      if (openOption === "false") {
        ...
        if (originalActiveLeaf && originalActiveLeaf.view) {
          ...
          const currentActiveView = app.workspace.getActiveViewOfType(import_obsidian7.MarkdownView);
...
var replace = async (app, { replace: replace2 }, position) => {
  await removeSection(app, replace2, position);
};
...
var findTemplate = (app, templateFile, templatesEnabled, templaterPluginEnabled) => {
  ...
  if (results.length === 0) {
    ...
    if (templaterResult && coreResult) {
      new import_obsidian9.Notice(`Found template "${templateFile}" in both Core Templates and Templater folders. Using Templater version.`, 3e3);
      return {
...
Searched in: ${availableFolders.join(", ")}` : "";
    new import_obsidian9.Notice(`Couldn't find template "${templateFile}.md" in any configured template folder.${folderList}`, 4e3);
    return;
  }
  ...
  try {
    ...
    new import_obsidian9.Notice(`Error executing template "${templateFile}". Check console for details.`, 3e3);
  }
};
